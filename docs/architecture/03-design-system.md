...
1. **Warmth carries the brand; restraint carries the credibility.** Cream, rounded corners and
   ...
   surface is both loud _and_ colourful, it reads as a toy catalogue.
...
| `--orange-700`  | `#A8430A` | 5.85:1 on paper                          | Orange used as _text_                                 |
...
| Display accent         | **Fraunces** (variable, `SOFT`/`WONK` axes, italic) | Delivers the brochure's italic-serif emphasis (_"Learning by Doing."_). Used **only** for emphasised phrases inside headings — never full paragraphs. |
...
```css
--shadow-sm: 0 1px 2px rgba(46, 38, 20, 0.05), 0 1px 3px rgba(46, 38, 20, 0.04);
--shadow-md:
  0 4px 12px rgba(46, 38, 20, 0.06), 0 2px 4px rgba(46, 38, 20, 0.04);
--shadow-lg:
  0 12px 32px rgba(46, 38, 20, 0.09), 0 4px 8px rgba(46, 38, 20, 0.04);
--shadow-xl: 0 24px 64px rgba(46, 38, 20, 0.12);
```

**Glass** — used sparingly: the sticky header on scroll, and the ⌘K palette. Nothing else.

```css
--glass: rgba(253, 251, 246, 0.72);
