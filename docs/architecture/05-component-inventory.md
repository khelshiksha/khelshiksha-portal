...
| `Mascot`         | RSC    | `crop: standing\|bust` · `size: sm\|md\|lg` · `alt?`                                                 | Brand mascot. Deliberately not via `MediaFrame` (fixed intrinsic ratio, not a ratio box). No `priority`, no `flipped` — the shirt reads "Play". Decorative by default. |
