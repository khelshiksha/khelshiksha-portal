...
>
> The budget is now set as _floor + headroom_: **199KB baseline + ~16KB of application code.**
...
`Reveal`, `Counter`, and the product-grid FLIP — but it is imported _only_ in components whose
...
const CommandPalette = dynamic(
  () => import("@/components/blocks/command-palette"),
);
const ImpactMap = dynamic(() => import("@/features/impact/impact-map"), {
  ssr: false,
});
const GalleryLightbox = dynamic(
  () => import("@/components/blocks/gallery-lightbox"),
);
...
3. **`priority` on exactly one image per page** (the LCP candidate) — and on the homepage that
   is deliberately _not_ the hero illustration, because the LCP element there is the headline
   text.
